...
async function fetchLiveConditions(lat, lon) {
    ...
    try {
        ...
        if (currentTime && hourlyTimes.length) {
            // current.time is 15-minutely ("13:30"), the hourly series on
            // the hour; an exact lookup failed most of each hour and fell
            // back to midnight. Take the current hour's entry instead.
            let startIndex = hourlyTimes.indexOf(currentTime.slice(0, 13) + ":00");
